...
class FacenetEngine(object):
    ...
    def recognize(self, image_path=None, image_data=None):
        """
        指定するイメージを認証する
        :param image_path: chosse 1 method input
        _ image_path = path input images
        _ image_data = image bytes (defaul None)
        ...
        if image_data is None:
            ...
            # print("img_encode : ", img_encode)
            print('in Encode image_path')
        else:
            errcode, img_encode = self.make_encode(image_path, image_data=image_data)
            print("REcognizeimg_encode : ", img_encode)
        ...
        if errcode is 0:
            ...
            #     # print("anchor : ", anchor)
            #     if len(anchor['encode']) == FacenetEngine.__encode_features_vector_length:      # __encode_features_vector_length=128
            #         _distance_scipy = distance.euclidean(img_encode, anchor['encode'])
            #         distances.append(_distance_scipy)
            #         if FacenetEngine.__debug_mode:
            #             print('Distance {} vs {} : {}'.format('You', anchor['name'], _distance_scipy))
            #     else:
            #         print("anchor encode : ", anchor['encode'])
            # print("Distance:", distances)
            # end_time = time()
            # print('Time use for calculate with scipy:' ,str(end_time-start_time))

            ### Testing for Calulate distance : np.linalg.norm'
            # print('[*] Testing for Calulate distance : np.linalg.norm')
            start_time = time()
            distances = list()       
            for anchor in all_anchors: 
                if len(anchor['encode']) == FacenetEngine.__encode_features_vector_length:
                    _distance = np.linalg.norm(img_encode - anchor['encode'], ord=2, axis=1)
                    distances.append(_distance)
                    print('[*] Distance {} vs {} : {}'.format('IMG', anchor['name'], _distance))
                else:
                    print("not equal 128 dim")       
            end_time = time()
            # print("Distance:", distances)
            print('Time use for calculate with numpy:' ,str(end_time-start_time))

            if FacenetEngine.__debug_mode is True:
                # print('type img_encode = {}'.format(type(img_encode)))
            ...
            if np.max(distances) < FacenetEngine.__bound:                   # __bound = 18
                if min(distances) < FacenetEngine.__distance_threshold:     # threshold = 11.0
                    ...
                    print("threshold: ", FacenetEngine.__distance_threshold)
    ...
    def extract_face_for_preprocessing(self, file_path, required_size=(160, 160)):
        ...
        print("Excute detected Face by MTCNN: ", end-start)
        if len(results) < 1:
            errcode = -1
        else:
            # extract the bounding box
    ...
    def encoding(model, faces):
        ...
        mean, std = faces.mean(), faces.std()                   # mean is average arrays
    ...
    def make_faces_encoding_labels(self):
        ...
        print("faces.shape: " , faces.shape)  # データ数, 160, 160, 3
        print("labels.shape:", labels.shape)  # データ数
    ...
    ### me
    def preprocessing_by_filepath(self, input_image):
        """
        Extract a face from input path a image and release it
        Args: 
            input_image (str)   :    path of image.jpeg (any size)
        Return: 
            image data          :    image (160*160*3) only a face released
        """
        ### get image name for extract path output
        filefull = os.path.split(input_image)[-1]                # header / [[filename.extenion]]
        print("filepath input preprocessing : ", filefull)

        filename, file_extension = os.path.splitext(filefull)
        # print("filename : ", filename)
        # print("file_extension : ", file_extension)

        if 'jpeg' in file_extension:
            errcode, face = self.extract_face_for_preprocessing(input_image)
            if errcode is 0:
                try:
                    # print("face : ", face)
                    pil_img = Image.fromarray(face)
                    pil_img.save(input_image)                   # replace current image with preprocessed image
                    return "Preprocessed img as in {}".format(input_image)

                except Exception as e:
                    print("preprocessing_by_filepath get error {}".format(e))
            else:
                print("preprocessing_by_filepath get error when extract face")
    ...
    def make_encode(self, input_image, image_data=None):
        ...
        if image_data is None:
            ...
            ### convert img 180*180*3 to 128 dim
            if errcode is 0:
                    faces = []
                    faces.append(face)
                    embed = self.convert(faces)
                    # print("make_encode embed with img_path : ", embed)
            else:
                print("error make encode with img_path: ", errcode)
        else:
            ...
            if face_img_receiver_mode:
                ...
                faces.append(pixels)
                embed = self.convert(faces)
                print("make_encode embed true : ", embed)
            else:
                ...
                if errcode is 0:
                    ...
                    faces.append(face)
                    embed = self.convert(faces)
                    print("make_encode embed false : ", embed)
...
if __name__ == "__main__":
    ...
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
    plt.legend(['train', 'validation'], loc='upper left')       # chú thích lines biểu đồ
    plt.show()
    # "Loss"
    plt.plot(train_loss)
    plt.plot(val_loss)
