...
class FacenetEngine(object):
    ...
    def train(self):
        ...
        # print("faces_encoding : ", faces_encoding)

        # Fit into SVM model
        model = SVC(kernel='linear', probability=True)                 
        # model = self.model
        # model.compile(optimizers.SGD(lr=1e-3, momentum=0.9), loss ='categorical_crossentropy', metrics=['accuracy'])
        model.fit(faces_encoding, normalized_labels)
        joblib.dump(model, self.__classifier_filename)
        print("Accuracy:" , model.score(faces_encoding, normalized_labels))
        print('Save')
    ...
    def transfer_learning(self, train_data_dir, validation_data_dir, epochs):
        ...
        # Initiate the train and test generators with data Augumentationsgdsgd
